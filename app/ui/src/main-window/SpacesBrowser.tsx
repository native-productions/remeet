                ...
                <li key={rec.id}>
                  ...
                  >
                    ...
                    <span className="folder-main">
                      <span className="folder-name">{folderName(rec.created)}</span>
                      <span className="folder-sub">
                        {duration(rec.duration_secs)} · {relativeTime(rec.created)}
                      </span>
                    </span>
                  </button>
                  ...
                  {isOpen && (
                    <div className="leaves">
                      <button
                        className={`leaf${
                          openedId === rec.id && openedView === "transcript"
                            ? " is-current"
                            : ""
                        }`}
                        type="button"
                        onClick={() => onOpen({ recording: rec, view: "transcript" })}
                      >
                        Transcript
                        {!rec.transcribed && <span className="leaf-note">not yet</span>}
                      </button>
                      <button
                        className={`leaf${
                          openedId === rec.id && openedView === "summary"
                            ? " is-current"
                            : ""
                        }`}
                        type="button"
                        onClick={() => onOpen({ recording: rec, view: "summary" })}
                      >
                        Summary
                        {!rec.summarized && <span className="leaf-note">not yet</span>}
                      </button>
                    </div>
                  )}
                </li>
